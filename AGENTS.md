...
(`node/src/party.rs`, SPEC §10.4 + §10.3, OPT-IN — the default drivers
...
H3 distributed committee ceremony (`node/src/ceremony.rs`) is the
...
- `cargo test -p ohm-ecdsa-node` — 89 tests: 3 M1 tests in
  ...
  and 4 H3 unit tests in `node/src/ceremony.rs` (identity/pub
  ...
  4 H5 unit tests in `node/src/locked.rs` (mlock fail-open policy) and
  6 in `node/src/seal.rs` (seal/open roundtrip, wrong-key/tamper/
...
- `cargo run -p ohm-ecdsa-node -- spawn-demo [--cheat-node K --cheat C]
  ...
  the concurrent-sessions demo: the H5 pool manager (`node/src/pool.rs`)
...
- **Presignatures are single-use and key-equivalent** (SPEC §8.6): `t`
  ...
  secrets (`node/src/locked.rs`, fail-open with a warning) and AEAD
  sealing at rest (`node/src/seal.rs`), but HSM-backed storage and

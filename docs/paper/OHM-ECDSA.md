...
**Draft v0.6 — for IACR ePrint submission. Unreviewed. Not audited. Typeset version: `main.pdf` (from `main.tex`). This paper is released under CC-BY-4.0; the code is Apache-2.0.**
...
3. **A security statement and proof skeleton (§5).** We state the intended theorem (realizing a threshold-ECDSA functionality against a static malicious adversary, in the random-oracle model under ECDLP, conditioned on a presignature-consumption invariant) and give a game-based proof outline with the two remaining gaps precisely identified (rushing-adversary DKG extraction; the Feldman-hiding hybrid).
...
- **HD key derivation.** For public tweak `τ`: `z′_j = z_j + τ·u_j`, `X′ = X + τ·G` — local, non-interactive, per the Groth–Shoup model. One presignature pool serves an entire BIP32-style tree. *Caveat:* AKD + presignatures is exactly the GS21 cube-root regime (~85-bit generic security when an adversary observes many pool nonces before choosing `(M, τ)`); deployments must bound outstanding records per key, never expose the pool to adaptive probing, and consume each record under exactly one `(key, tweak)` pair (SPEC §9.4).

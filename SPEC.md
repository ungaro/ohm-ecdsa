...
8. ~~The re-randomization lemma~~ — **proved (proof §8.2.6)** modulo one named heuristic (F-uniformity of the x-coordinate map — the standard assumption this lineage's analyses, including the GS21 attack itself, already make). The cube-root attack's enabler is the constant-`r` affine structure; re-randomization destroys it (single-query fixed point at `O(q)`; multi-candidate collision in `Φ(M, τ) = h(M) + F(γ·R)·τ` at birthday `Θ(√q)`; scheduling bounded by `q_H²/2q + q_s·q_H/q`). This makes OHM-ECDSA the second honest-majority scheme with a mitigation for the GS21 attack surface (after [^gs22svc]) and the only one compatible with direct inverse-dealing; DJNPO20 and KU24 have none. Remaining: external review of the lemma, and the §7.3 bookkeeping pages elsewhere in the proof.

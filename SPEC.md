...
8. **The re-randomization lemma.** The standard additive mitigation (`k′ = k + δ`) is foreclosed by direct inverse-dealing (§9.4: no local formula for `(k+δ)⁻¹` from `k⁻¹` shares). Multiplicative re-randomization `k′ = γk` with `γ = H(sid ‖ id ‖ M ‖ τ ‖ X)` is the inverse-compatible candidate. **Progress (proof §8.2):** the cube-root attack's enabler is the *constant*-`r` affine structure, which re-randomization destroys — the single-query forgery condition is an unstructured RO fixed point at `O(q)`, and the multi-candidate attack is a collision in the effectively-random map `Φ(M, τ) = h(M) + F(γ·R)·τ`, whose generic (birthday) cost is `Θ(√q)` — the square-root bound restored (proof §8.2.5, including the observation that `k′ = H(M‖τ‖X)·k` is an RFC-6979-style message-derived nonce, so the attack's pre-computation window ceases to exist in the model). The residual is two bookkeeping items (proof §8.2.5 step 4): a genericity lemma for `γ ↦ F(γ·R)` in the AGM, and adaptive-scheduling bookkeeping à la Groth–Shoup. Completing them would make this the second honest-majority scheme with a mitigation for the GS21 attack surface (after [^gs22svc]); DJNPO20 and KU24 have none.

...
8. **The re-randomization lemma.** The standard additive mitigation (`k′ = k + δ`) is foreclosed by direct inverse-dealing (§9.4: no local formula for `(k+δ)⁻¹` from `k⁻¹` shares). Multiplicative re-randomization `k′ = γk` with `γ = H(sid ‖ id ‖ M ‖ τ ‖ X)` is the inverse-compatible candidate. **Progress (proof §8.2):** the single-query core is proved — the cube-root attack's enabler is the *constant*-`r` affine structure, which re-randomization destroys, leaving an unstructured RO fixed point at `O(q)` generic cost (above the square-root baseline). The named remaining gap is the multi-query lifting through the Groth–Shoup AGM framework (proof §8.2.4(ii)). This is a standalone lemma with value beyond OHM-ECDSA: completing it would make this the second honest-majority scheme with a mitigation for the GS21 attack surface (after [^gs22svc]); DJNPO20 and KU24 have none.

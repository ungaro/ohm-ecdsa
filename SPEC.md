...
**The mitigation question — a structural constraint, stated honestly.** Groth–Shoup's own mitigation is *additive* re-randomization (`k′ = k + δ`), which is local when parties hold `[k]`. OHM-ECDSA deliberately does **not** hold `[k]` — it holds `[u] = [k⁻¹]` (the direct-dealing design-around of §8/§12) — and `(k + δ)⁻¹` has no local formula in `k⁻¹` shares: computing it requires precisely the inversion protocol the design-around exists to avoid. **The design-around forecloses the standard mitigation.** (As far as we know, this consequence of inverse-dealing has not been recorded elsewhere; it is worth recording.) The only inverse-compatible alternative is **multiplicative** re-randomization: `γ = H(sid ‖ id ‖ M ‖ τ ‖ X)`, `k′ = γk`, `R′ = γR`, `u′ = γ⁻¹u`, `z′ = γ⁻¹z` — every step a local scalar scaling, commitments scaling by the same public factors, one-round online preserved, and no claim-chart element touched (no `[w]`, no authenticator, no zero-sharing). **Status: proof sketch (proof §8.2.6) — every attack case analyzed, modulo one named heuristic (G1, F-uniformity) and a GGM model note.** The concern was that an RO-derived `γ` lets the adversary evaluate `r′(M, τ) = F(γ·R)` offline for *every* candidate `(M, τ)` pair — strictly more power than a consensus-derived `δ`. The analysis in `docs/proof/PROOF.md` §8.2 shows this is not the attack's real enabler: the cube-root attack needs the *affine* structure that only a **constant** `r` provides, and re-randomization destroys it — the single-query condition is an unstructured RO fixed point at `O(q)`, the multi-candidate collision in `Φ(M, τ) = h(M) + F(γ·R)·τ` costs birthday `Θ(√q)`, and the genericity/scheduling lemmas (§8.2.6) bound the full adaptive game by `q_H²/2q + q_s·q_H/q + Adv_GS-EUF` — square-root security restored. The one named assumption is F-uniformity of the x-coordinate map, the standard heuristic this lineage (including the GS21 attack itself) already makes. Pending external review, the mitigation stays experimental and default-off, and SPEC §9.4 policy (signing-time disclosure, bounded pools) remains the security posture. The candidate is implemented in the reference implementation behind an EXPERIMENTAL, default-off API (`sign::rerand_gamma`, `sign::sign_share_rerand`, `sign::combine_rerand`, `sim::run_sign_rerand`) so the construction can be analyzed and tested — its status is unchanged: open lemma, not for production use.
...
**Status: structured sketch, not a proof.** This section states the intended security theorem, decomposes it into proof obligations C1–C6, and gives for each an explicit argument skeleton (simulator construction or reduction) rather than informal intuition. What separates this from a proof is enumerated precisely in §11.3, and a game-based proof outline is given in §11.4. **The full proof write-up lives in `docs/proof/PROOF.md`** (working document): C1, C3, C4, C6, the abort-distribution lemma (§11.3(4)), L1, L2's AGM form, the composition lemma, and the game-based security claim are proved there in the AGM+ROM under ECDLP and the Groth–Shoup presignature-ECDSA assumption; the re-randomization lemma (§11.3(8)) is a proof sketch with all attack cases analyzed (one named heuristic + a GGM model note); UC is roadmapped (§9 there) with Theorem U3 proved at hybrid-argument level. A full game-based or UC treatment remains future work and is a prerequisite to production use, alongside independent implementation review.
...
4. ~~Abort-leakage accounting~~ — **closed in proof §8.4**: every abort trigger is a boolean predicate over public verification outcomes (hash match, point-equality, DLEQ, signature checks, public zero-value tests, §10.5), so the simulator reproduces the abort distribution — occurrence, phase, and blamed set — exactly, up to the per-hop bounds already charged; no separate leakage term.
...
8. ~~The re-randomization lemma~~ — **proof sketch; all cases analyzed** (proof §8.2). The cube-root attack's enabler is the constant-`r` affine structure; re-randomization destroys it (single-query fixed point at `O(q)`; multi-candidate collision in `Φ(M, τ) = h(M) + F(γ·R)·τ` at birthday `Θ(√q)` with the algebraic handles accounted via two-block MITM; and the no-collision residual is closed by conditional reduction to Groth–Shoup's Theorem 6 via their entropy-preservation generalization — total bound `O(q_H²/q) + O(q_s·q_H/q) + Adv_GS21-Thm6`). Named soft spots: the F-uniformity heuristic (G1, also required by the attack being mitigated) and a model note (the case-(3) plug-in is in the GGM while cases (1)–(2) are ROM — the standard two-layer composition). This makes OHM-ECDSA the second honest-majority scheme with a mitigation for the GS21 attack surface (after [^gs22svc]) and the only one compatible with direct inverse-dealing; DJNPO20 and KU24 have none. Remaining: external review.
...
with `ε_L1` the DKG-extraction gap (§11.3(1)) and `ε_L2` the hiding-hybrid gap (§11.3(2)). Robustness (C6) and abort-distribution indistinguishability (§11.3(4)) are **proved in proof §8.4**: every abort decision is a function of public verification outcomes, and continuation uses only publicly committed values.
...
**Recommendation.** Adopt AGM+ROM for the full proof (option 1): it subsumes the model GS21 already requires for C5, and it leaves ECDLP as the sole hardness assumption in the final theorem statement. §11.3(2) is updated accordingly; L2's parts are now written in the proof document (proof §7: (a) DL-equivalent, proved; (b) a named `(T−1)`-OMDL assumption, not proved; (c) AGM form proved in §7.3 — representation extractor, basis induction, and the simultaneous-embedding argument that avoids the factor-`Q` union loss, all written).

...
The full mitigation is GS21-style **multiplicative nonce re-randomization** at signing time: `δ = H(R ‖ M ‖ τ)`, `k′ = δk`, `R′ = δR`, `u′ = δ⁻¹u`, `z′ = δ⁻¹z + τ·u′` — every step is local scalar scaling, so it composes with this protocol's linear structure and with the commitment-bound checks (commitments scale by the same public `δ⁻¹`). It is specified as future work (§11.3(7)); until it lands, HD tweaks carry the stated bound and the policies above are the security posture.
...
* At rest, the reference node seals every secret file (presignature store records — key-equivalent per §8.6(2) — seed/identity files) inside a ChaCha20-Poly1305 envelope under a per-node storage key (`node/src/store/seal.rs`), written `0600`; legacy cleartext files are rejected (fail closed, no silent downgrade). The storage key resolves from `OHM_STORAGE_KEY` / `OHM_STORAGE_KEY_FILE` / a generated `0600` dev key file — this is the **interface** for a KMS/HSM, not a KMS: key custody, rotation, and rollback defense (a malicious host restoring deleted sealed files) remain deployment concerns.
